/* Follow The Tracks — clinical reasoning simulator (frontend-only POC).
   React (CDN) + in-memory state with an optional browser-store cache for saved attempts. */
...
/* ---- safe storage: in-memory by default; persists to the browser store where the
   environment allows it. Accessed indirectly so sandboxed preview iframes (which
   disallow persistent storage) fall back cleanly to in-memory without crashing. ---- */
const memStore = {};
...
const store = {
  get(k) {
    try { if (backing) { const v = backing.getItem(k); return v ? JSON.parse(v) : (memStore[k] ?? null); } }
    catch (e) {}
    return memStore[k] ?? null;
  },
  set(k, v) {
    memStore[k] = v;
    try { if (backing) backing.setItem(k, JSON.stringify(v)); } catch (e) {}
  },
};
...
function Logo({ className }) {
  // Abstract lens + pulse hybrid: a ring (observation) crossed by a reasoning line
  return h("svg", { className, viewBox: "0 0 40 40", fill: "none", "aria-label": "Follow The Tracks" },
...
function IconLock(p) {
  return h("svg", { ...p, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2 },
    h("rect", { x: 4, y: 11, width: 16, height: 9, rx: 2 }),
    h("path", { d: "M8 11V7a4 4 0 0 1 8 0v4" }));
}
function IconCheck(p) {
  return h("svg", { ...p, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2.6, strokeLinecap: "round", strokeLinejoin: "round" },
    h("path", { d: "M20 6 9 17l-5-5" }));
}
function IconX(p) {
  return h("svg", { ...p, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2.6, strokeLinecap: "round" },
    h("path", { d: "M18 6 6 18M6 6l12 12" }));
}
function IconArrow(p) {
  return h("svg", { ...p, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round" },
    h("path", { d: "M5 12h14M13 6l6 6-6 6" }));
}

const USER = { name: "Dr. Priya Raghavan", sub: "JIPMER · 2nd yr MD Medicine", initials: "PR" };
...
function App() {
  ...
  const enterApp = () => { setSignedIn(true); setShowSignin(false); nav("/cases"); };

  // route guard: app routes require sign-in
  const isAppRoute = route.startsWith("/cases") || route.startsWith("/case/") || route.startsWith("/report/");
  useEffect(() => {
    if (isAppRoute && !signedIn) setSignedIn(true); // deep-link → auto sign in (mock)
  }, [isAppRoute, signedIn]);
...
function TopBar({ nav, onSignin }) {
  return h("header", { className: "topbar" },
    h("div", { className: "wrap topbar-inner" },
      h("a", { className: "brand", href: "#/", onClick: (e) => { e.preventDefault(); nav("/"); } },
...
function AppHeader({ nav }) {
  return h("header", { className: "app-header" },
    h("div", { className: "wrap app-header-inner" },
      h("a", { className: "brand", href: "#/cases", onClick: (e) => { e.preventDefault(); nav("/cases"); } },
        h(Logo, { className: "brand-mark" }),
        h("span", { className: "brand-name" }, "Follow The Tracks"),
      ),
      h("div", { className: "user-badge" },
        h("div", { className: "user-meta", style: { textAlign: "right" } },
...
function Landing({ nav, onSignin, onTry }) {
  return h(React.Fragment, null,
    ...
    // how it works
    h("section", { id: "how", className: "section section-alt" },
    ...
    // what it measures
    h("section", { id: "scoring", className: "section" },
      h("div", { className: "wrap" },
        h("div", { className: "section-head" },
    ...
    // pricing
    h("section", { id: "pricing", className: "section section-alt" },
      h("div", { className: "wrap" },
        h("div", { className: "section-head" },
        ...
        h("div", { className: "price-grid" },
          h(PriceCard, { name: "Department", amt: "₹900", unit: "/ student / year", desc: "For a single department piloting the platform.", feature: false,
            items: ["Full 3-case starter bank", "Unlimited attempts & reports", "Per-student progress", "Email support"], cta: "Start a pilot", onCta: onTry }),
          h(PriceCard, { name: "College", amt: "₹650", unit: "/ student / year", desc: "The usual choice — whole college, all clinical years.", feature: true,
            items: ["Everything in Department", "Growing specialty case bank", "Department analytics for HODs", "Cohort benchmarking", "Custom cases on request"], cta: "Talk to us", onCta: onSignin }),
          h(PriceCard, { name: "University", amt: "Custom", unit: "multi-college", desc: "For universities and health-science networks.", feature: false,
            items: ["Everything in College", "Multi-college dashboards", "SSO & roster integration", "Dedicated academic liaison"], cta: "Request a quote", onCta: onSignin }),
        ),
      ),
    ),
    // footer
    h("footer", { className: "footer" },
      h("div", { className: "wrap footer-inner" },
        ...
        h("p", null, "© 2026 Follow The Tracks. A demonstration build."),
      ),
    ),
  );
}
...
function HeroSpecimen() {
  return h("div", { className: "specimen" },
    h("div", { className: "specimen-head" },
      h("span", { className: "lbl" }, "Sample · Reasoning report"),
      h("span", { className: "lbl" }, "Case 1"),
    ),
    h("div", { className: "specimen-body" },
...
function CaseLibrary({ nav, attempts }) {
  ...
  return h("div", { className: "wrap section" },
    ...
    !any && h("div", { className: "empty", style: { marginBottom: "2rem" } },
...
function CaseCard({ c, attempt, nav }) {
  return h("div", { className: "case-card" },
    h("div", { className: "case-card-top" },
      h("div", { className: "case-meta-row" },
    ...
    h("div", { className: "case-card-foot" },
      h("div", { className: "case-stats" },
        h("span", null, "~" + c.minutes + " min"),
        attempt
          ? h("span", { className: "badge-attempted" }, h(IconCheck, { width: 13, height: 13 }), "Scored " + attempt.result.overall)
          : h("span", null, "Not attempted"),
      ),
      attempt
        ? h("button", { className: "btn btn-ghost", onClick: () => nav("/report/" + c.id), style: { padding: "0.5em 1em" } }, "View report")
        : h("button", { className: "btn btn-primary", onClick: () => nav("/case/" + c.id), style: { padding: "0.5em 1em" } }, "Begin", h(IconArrow, { width: 15, height: 15 })),
    ),
  );
}
...
function CaseAttempt({ caseId, nav, saveAttempt }) {
  ...
  useEffect(() => { taRef.current?.focus(); }, [stage]);

  if (!caseData) {
    return h("div", { className: "wrap section" }, h("p", null, "Case not found."),
  ...
  return h("div", { className: "wrap section" },
    h("div", { className: "attempt-layout" },
      ...
      // panel
      h("section", { className: "stage-panel", key: stage },
        h("div", { className: "stage-panel-head" },
          h("div", { className: "stage-kicker" }, "Stage " + (stage + 1) + " of 5 · " + STAGE_LABELS[stage]),
          h("h2", null, stageTitle(stage)),
        ),
        h("div", { className: "stage-panel-body fade-in" },
          // content by stage
          ...
          // prompt + textarea
          h("div", { className: "prompt-box" },
            h("p", { className: "prompt-q" }, prompts[stage]),
            h("textarea", {
              ref: taRef, className: "answer", value: draft,
              onChange: (e) => setDraft(e.target.value),
              placeholder: placeholderFor(stage),
            }),
            h("div", { className: "answer-meta" },
              h("span", { className: "word-count" }, wc + " words"),
              h("span", { className: "commit-note" }, h(IconLock, { width: 12, height: 12 }),
                stage < 4 ? "Once submitted, this stage locks." : "Submitting grades your attempt."),
            ),
            h("div", { style: { marginTop: "1.25rem", display: "flex", justifyContent: "flex-end", gap: "0.75rem" } },
              h("button", { className: "btn btn-primary btn-lg", onClick: submitStage, disabled: !canSubmit },
...
function VignetteView({ v }) {
  return h("div", { className: "vignette" },
    h("h4", null, "History"),
    h("p", null, v.history),
    h("h4", null, "Vitals on arrival"),
    h("div", { className: "vitals-grid" },
      v.vitals.map((vt, i) => h("div", { className: "vital", key: i },
...
function PriorRecap({ stage, answers, caseData }) {
  // Show read-only recap of the immediately relevant prior commit
  if (stage === 1) {
    return h("div", { className: "locked-recap" },
...
function ResultsView({ caseData, answers }) {
  ...
  return h(React.Fragment, null,
    ...
    r.tables.map((t, i) => h("div", { className: "lab-table-wrap", key: i },
      h("div", { className: "lab-table-title" }, t.title),
      h("table", { className: "lab" },
        h("thead", null, h("tr", null,
          h("th", null, "Test"), h("th", null, "Value"), h("th", null, "Reference"))),
        h("tbody", null, t.rows.map((row, j) => h("tr", { key: j },
          h("td", null, row.test),
          h("td", { className: "val" }, row.value,
...
function Report({ caseId, attempts, nav }) {
  ...
  if (!caseData || !attempt) {
    return h("div", { className: "wrap section" }, h("p", null, "No report yet."),
  ...
  return h("div", { className: "wrap section" },
    ...
    h("div", { className: "panel full-width", style: { marginBottom: "2rem" } },
      h("div", { className: "prof-feedback" },
    ...
    h("div", { className: "report-grid" },
      ...
      h("div", { style: { display: "flex", flexDirection: "column", gap: "2rem" } },
        h("div", { className: "panel" },
          ...
          h("div", { className: "killers" },
            result.raw.reasoning.caught.map((k, i) => h("div", { className: "killer-row", key: "c" + i },
              h("span", { className: "killer-ic caught" }, h(IconCheck, { width: 12, height: 12 })),
              h("span", null, k.label,
                h("span", { className: "understood" }, "  · read from “" + k.understoodAs + "”")),
            )),
            result.raw.reasoning.missed.map((m, i) => h("div", { className: "killer-row", key: "m" + i },
              h("span", { className: "killer-ic missed" }, h(IconX, { width: 12, height: 12 })),
              h("span", null, m, h("span", { className: "understood" }, "  · not raised at stage 1")),
            )),
          ),
        ),
        h("div", { className: "panel" },
          h("h2", null, "How we read your words"),
          h("div", { className: "mapping" },
            buildMapping(caseData, attempt).map((m, i) => h("div", { className: "map-row", key: i },
        ...
        h("div", { className: "panel" },
          ...
          result.gaps.length
            ? h("ul", { className: "gaps-list" }, result.gaps.map((g, i) => h("li", { key: i }, g)))
            : h("p", { style: { fontSize: "var(--text-sm)", color: "var(--green)" } }, "No significant gaps flagged. Clean work."),
        ),
      ),
    ),
...
// derive a few transparent mappings from the differentials the student actually wrote
function buildMapping(caseData, attempt) {
  ...
  for (const d of caseData.rubric.differentials) {
    for (const syn of d.synonyms) {
      ...
      // show only non-obvious (colloquial) mappings, not the exact label
      if (raw.includes(s) && s !== d.label.toLowerCase() && !seen.has(d.id)) {
        // prefer a colloquial phrase (contains a space or is short slang)
...
function ScoreRing({ value, color }) {
  ...
  return h("div", { style: { position: "relative", width: 130, height: 130 } },
    h("svg", { width: 130, height: 130, viewBox: "0 0 130 130" },
      h("circle", { cx: 65, cy: 65, r: R, fill: "none", stroke: "var(--line)", strokeWidth: 9 }),
      h("circle", { cx: 65, cy: 65, r: R, fill: "none", stroke: color, strokeWidth: 9,
        strokeLinecap: "round", strokeDasharray: C, strokeDashoffset: off,
        transform: "rotate(-90 65 65)", style: { transition: "stroke-dashoffset 900ms cubic-bezier(0.16,1,0.3,1)" } })),
...
function Radar({ dimensions }) {
  ...
  return h("svg", { width: size, height: size + 30, viewBox: `0 0 ${size} ${size + 30}`, style: { overflow: "visible" } },
    rings.map((rr, i) => h("polygon", { key: "r" + i, fill: "none", stroke: "var(--line)", strokeWidth: 1,
      points: dimensions.map((_, j) => pt(j, R * rr).join(",")).join(" ") })),
    dimensions.map((_, i) => { const [x, y] = pt(i, R); return h("line", { key: "a" + i, x1: cx, y1: cy, x2: x, y2: y, stroke: "var(--line)", strokeWidth: 1 }); }),
    ...
    dimensions.map((d, i) => { const [x, y] = pt(i, R * (d.score / 100)); return h("circle", { key: "d" + i, cx: x, cy: y, r: 3, fill: "var(--accent)" }); }),
    dimensions.map((d, i) => {
      const [x, y] = pt(i, R + 20);
      const anchor = Math.abs(x - cx) < 10 ? "middle" : x > cx ? "start" : "end";
      return h("text", { key: "t" + i, x, y, textAnchor: anchor, dominantBaseline: "middle",
        fontSize: 11, fill: "var(--ink-soft)", fontFamily: "var(--font-body)" }, short[d.key] || d.key);
    }),
  );
}
